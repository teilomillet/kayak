...
Candidate-window policy finding: a benchmark-only policy,
...
The measured bottleneck was candidate coverage. The `1.25x` rule is still
benchmark-only, but it is less brittle than the minimum observed repair
(`candidate_k=260`) and stays inside the existing speed margin.
...
45. Add a benchmark-only candidate-window policy for document-vector-heavy
