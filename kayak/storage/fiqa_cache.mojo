...
from kayak.interop import load_fiqa_real_subset
...
struct FiqaRealSubsetCache(Copyable):
...
def build_fiqa_stored_task(
    ...
    return StoredJudgedTask(
        ...
        model_name.copy(),
        VECTOR_SCALAR_NAME,
        load_fiqa_real_subset(query_limit, negative_doc_limit, model_name),
    )
...
def load_or_build_fiqa_stored_task(
    task_root: Path,
    query_limit: Int,
    negative_doc_limit: Int,
    model_name: String,
    ...
    var stored_task = build_fiqa_stored_task(
...
def ensure_fiqa_real_subset_cache(
    cache_root: Path = ".cache/kayak/fiqa_real_subset",
    query_limit: Int = 6,
    negative_doc_limit: Int = 64,
    model_name: String = "colbert-ir/colbertv2.0",
) raises -> FiqaRealSubsetCache:
    ...
    var stored_task = load_or_build_fiqa_stored_task(
