...
from kayak.interop import load_lemb_narrativeqa_real_subset
...
struct LembNarrativeQaRealSubsetCache(Copyable):
...
def build_lemb_narrativeqa_stored_task(
    query_limit: Int,
    model_name: String,
) raises -> StoredJudgedTask:
    return StoredJudgedTask(
        "mteb/LEMBNarrativeQARetrieval/test",
        model_name.copy(),
        VECTOR_SCALAR_NAME,
        load_lemb_narrativeqa_real_subset(
            query_limit,
            model_name,
        ),
    )
...
def load_or_build_lemb_narrativeqa_stored_task(
    task_root: Path,
    query_limit: Int,
    model_name: String,
) raises -> StoredJudgedTask:
    ...
    var stored_task = build_lemb_narrativeqa_stored_task(
        query_limit,
        model_name,
    )
...
def ensure_lemb_narrativeqa_real_subset_cache(
    cache_root: Path = ".cache/kayak/lemb_narrativeqa_real_subset",
    query_limit: Int = 8,
    model_name: String = "colbert-ir/colbertv2.0",
) raises -> LembNarrativeQaRealSubsetCache:
    ...
    var stored_task = load_or_build_lemb_narrativeqa_stored_task(
        task_root,
        query_limit,
        model_name,
    )
    ...
    return LembNarrativeQaRealSubsetCache(
        stored_task^,
        stored_index^,
