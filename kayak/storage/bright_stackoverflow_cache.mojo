...
from kayak.interop import load_bright_stackoverflow_real_subset
...
struct BrightStackoverflowRealSubsetCache(Copyable):
...
def build_bright_stackoverflow_stored_task(
    query_limit: Int,
    negative_doc_limit: Int,
    model_name: String,
) raises -> StoredJudgedTask:
    return StoredJudgedTask(
        "xlangai/BRIGHT/stackoverflow",
        model_name.copy(),
        VECTOR_SCALAR_NAME,
        load_bright_stackoverflow_real_subset(
            query_limit,
            negative_doc_limit,
            model_name,
        ),
...
def load_or_build_bright_stackoverflow_stored_task(
    task_root: Path,
    query_limit: Int,
    negative_doc_limit: Int,
    model_name: String,
    ...
    var stored_task = build_bright_stackoverflow_stored_task(
        query_limit,
        negative_doc_limit,
        model_name,
    )
...
def ensure_bright_stackoverflow_real_subset_cache(
    cache_root: Path = ".cache/kayak/bright_stackoverflow_real_subset",
    query_limit: Int = 8,
    negative_doc_limit: Int = 128,
    model_name: String = "colbert-ir/colbertv2.0",
) raises -> BrightStackoverflowRealSubsetCache:
    ...
    var stored_task = load_or_build_bright_stackoverflow_stored_task(
        task_root,
        query_limit,
        negative_doc_limit,
        model_name,
    ...
    return BrightStackoverflowRealSubsetCache(
        stored_task^,
        stored_index^,
