...
from kayak.interop import load_limit_small_real_subset
...
struct LimitSmallRealSubsetCache(Copyable):
...
def build_limit_small_stored_task(
    query_limit: Int,
    model_name: String,
    ...
    return StoredJudgedTask(
        ...
        model_name.copy(),
        VECTOR_SCALAR_NAME,
        load_limit_small_real_subset(query_limit, model_name),
    )
...
def load_or_build_limit_small_stored_task(
    task_root: Path,
    query_limit: Int,
    model_name: String,
) raises -> StoredJudgedTask:
    ...
    var stored_task = build_limit_small_stored_task(query_limit, model_name)
...
def ensure_limit_small_real_subset_cache(
    cache_root: Path = ".cache/kayak/limit_small_real_subset",
    query_limit: Int = 32,
    model_name: String = "colbert-ir/colbertv2.0",
) raises -> LimitSmallRealSubsetCache:
    ...
    var stored_task = load_or_build_limit_small_stored_task(
        task_root, query_limit, model_name
    )
    ...
    return LimitSmallRealSubsetCache(
        stored_task^,
        stored_index^,
