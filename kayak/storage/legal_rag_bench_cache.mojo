...
from kayak.interop import load_legal_rag_bench_real_subset
...
struct LegalRagBenchRealSubsetCache(Copyable):
...
def load_or_build_legal_rag_bench_stored_task(
    task_root: Path,
    query_limit: Int,
    negative_doc_limit: Int,
    model_name: String,
    ...
    var stored_task = build_legal_rag_bench_stored_task(
        query_limit,
        negative_doc_limit,
        model_name,
    )
...
def ensure_legal_rag_bench_real_subset_cache(
    cache_root: Path = ".cache/kayak/legal_rag_bench_real_subset",
    query_limit: Int = 8,
    negative_doc_limit: Int = 128,
    model_name: String = "colbert-ir/colbertv2.0",
) raises -> LegalRagBenchRealSubsetCache:
    ...
    var stored_task = load_or_build_legal_rag_bench_stored_task(
        task_root,
        query_limit,
        negative_doc_limit,
        model_name,
    ...
    return LegalRagBenchRealSubsetCache(
        stored_task^,
        stored_index^,
