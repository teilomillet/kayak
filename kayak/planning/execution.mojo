...
def candidate_generation_for_plan[Backend: ExactScoringBackend](
    ...
    if plan.candidate_generator.kind == "centroid_postings_flat":
        var vector_count = 0
        var token_count = 0
        var byte_size = 0
        ...
        for segment in snapshot.segments:
            if not segment.has_centroid_postings_index:
                raise Error(
                    "centroid_postings_flat stage-1 requires a centroid postings sidecar for every segment"
                )
            ...
            var scores = centroid_posting_flat_scores_for_segment(
                query,
                segment.stored_centroid_postings_index.index,
            )
    ...
    if plan.candidate_generator.kind == "centroid_postings_head_auto":
        var vector_count = 0
        var token_count = 0
        var byte_size = 0
        ...
        for segment in snapshot.segments:
            if not segment.has_centroid_postings_index:
                raise Error(
                    "centroid_postings_head_auto stage-1 requires a centroid postings sidecar for every segment"
                )
            if (
                segment.stored_centroid_postings_index.posting_order_kind
                != CENTROID_POSTINGS_ORDER_WEIGHT_DESC_DOC_ASC
                ...
                raise Error(
                    "centroid_postings_head_auto stage-1 requires weight-sorted centroid postings sidecars"
                )
            ...
            var scores = centroid_posting_head_auto_scores_for_segment(
                query.token_vectors,
                segment.stored_centroid_postings_index.index,
                plan.candidate_budget.candidate_k,
            )
    ...
    if plan.candidate_generator.kind == "centroid_postings_blockmax":
        var vector_count = 0
        var token_count = 0
        var byte_size = 0
        ...
        for segment in snapshot.segments:
            if not segment.has_centroid_postings_index:
                raise Error(
                    "centroid_postings_blockmax stage-1 requires a centroid postings sidecar for every segment"
                )
            if (
                segment.stored_centroid_postings_index.posting_order_kind
                != CENTROID_POSTINGS_ORDER_WEIGHT_DESC_DOC_ASC
                ...
                raise Error(
                    "centroid_postings_blockmax stage-1 requires weight-sorted centroid postings sidecars"
                )
            ...
            var scores = centroid_posting_blockmax_scores_for_segment(
                query.token_vectors,
                segment.stored_centroid_postings_index.index,
                plan.candidate_budget.candidate_k,
            )
    ...
    if plan.candidate_generator.kind == "centroid_postings_imputed_flat":
        var vector_count = 0
        var token_count = 0
        var byte_size = 0
        ...
        for segment in snapshot.segments:
            if not segment.has_centroid_postings_index:
                raise Error(
                    "centroid_postings_imputed_flat stage-1 requires a centroid postings sidecar for every segment"
                )
            ...
            var scores = centroid_posting_imputed_flat_scores_for_segment(
                query,
                segment.stored_centroid_postings_index.index,
                plan.candidate_budget.final_k,
            )
