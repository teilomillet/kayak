...
comptime TOKEN_POOLING_SEARCH_MIN_SECONDS = 0.05
comptime TOKEN_POOLING_SEARCH_MAX_SECONDS = 0.25
comptime TOKEN_POOLING_SEARCH_MAX_ITERS = 200


struct TokenPoolingSummary(Copyable):
...
def build_token_pooling_summary(
    ...
    var evaluation = evaluate_task_on_index(backend, task, pooled_index.index)
    ...
    for judged_query in task.queries:
        ...
        var pooled_hits = search_exact(
            backend, judged_query.query, pooled_index.index, task.k
        )
        reference_recall_total += reference_recall_at_k(
            reference_hits, pooled_hits, task.k
        )
    ...
    var search_report = run[search_once](
        num_warmup_iters=1,
        max_iters=TOKEN_POOLING_SEARCH_MAX_ITERS,
        min_runtime_secs=TOKEN_POOLING_SEARCH_MIN_SECONDS,
        max_runtime_secs=TOKEN_POOLING_SEARCH_MAX_SECONDS,
    )

    return TokenPoolingSummary(
...
def append_token_pooling_summary_json(
    mut buffer: String, read summary: TokenPoolingSummary
):
    buffer += "{"
...
def token_pooling_summary_json(read summary: TokenPoolingSummary) -> String:
    var buffer = String()
    append_token_pooling_summary_json(buffer, summary)
    return buffer^


def token_pooling_summaries_json(read summaries: List[TokenPoolingSummary]) -> String:
    var buffer = String()
    buffer += "["
    ...
    for index in range(len(summaries)):
        if index > 0:
            buffer += ","
        append_token_pooling_summary_json(buffer, summaries[index])
