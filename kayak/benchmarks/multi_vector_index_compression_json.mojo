...
comptime MULTI_VECTOR_INDEX_COMPRESSION_SEARCH_MIN_SECONDS = 0.05
comptime MULTI_VECTOR_INDEX_COMPRESSION_SEARCH_MAX_SECONDS = 0.25
comptime MULTI_VECTOR_INDEX_COMPRESSION_SEARCH_MAX_ITERS = 200


struct MultiVectorIndexCompressionSummary(Copyable):
...
def standard_multi_vector_index_compression_budget_sizes(
    max_budget: Int
) -> List[Int]:
    return standard_vector_pruning_budget_sizes(max_budget)


def build_transformed_stored_index_for_multi_vector_index_compression(
...
def build_multi_vector_index_compression_summary(
    ...
    method_kind: String,
    requested_document_vector_budget: Int,
) raises -> MultiVectorIndexCompressionSummary:
    _ = require_multi_vector_index_compression_method_stored_representation_executable(
        method_kind
    )
    ...
    var transformed_index = (
        build_transformed_stored_index_for_multi_vector_index_compression(
            stored_task,
            full_index,
            spec,
        )
    ...
    var search_report = run[search_once](
        num_warmup_iters=1,
        max_iters=MULTI_VECTOR_INDEX_COMPRESSION_SEARCH_MAX_ITERS,
        min_runtime_secs=MULTI_VECTOR_INDEX_COMPRESSION_SEARCH_MIN_SECONDS,
        max_runtime_secs=MULTI_VECTOR_INDEX_COMPRESSION_SEARCH_MAX_SECONDS,
    )

    return MultiVectorIndexCompressionSummary(
        stored_task.dataset_id.copy(),
        stored_task.model_name.copy(),
        task.family.copy(),
...
def append_multi_vector_index_compression_summary_json(
    mut buffer: String, read summary: MultiVectorIndexCompressionSummary
):
    buffer += "{"
    ...
    buffer += "\"execution_boundary\":\""
    buffer += json_escape(summary.execution_boundary) + "\","
...
def multi_vector_index_compression_summary_json(
    read summary: MultiVectorIndexCompressionSummary
) -> String:
    var buffer = String()
    append_multi_vector_index_compression_summary_json(buffer, summary)
    return buffer^


def multi_vector_index_compression_summaries_json(
    read summaries: List[MultiVectorIndexCompressionSummary]
) -> String:
    var buffer = String()
    buffer += "["
    ...
    for index in range(len(summaries)):
        if index > 0:
            buffer += ","
        append_multi_vector_index_compression_summary_json(
