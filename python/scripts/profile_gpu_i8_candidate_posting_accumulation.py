...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(".cache/kayak/gpu_i8_candidate_posting_accumulation/summary.json"),
    )
...
def print_quiet_sections(report: dict[str, Any]) -> None:
    ...
    for row in rows:
        ...
        if not isinstance(name, str) or not isinstance(comparison, dict):
            continue
        print_quiet_mean(
            f"gpu_i8_candidate_posting_accumulation_{name}_cpu_candidate_generation",
            comparison.get("cpu_i8_candidate_generation_mean_seconds"),
        )
        print_quiet_mean(
            f"gpu_i8_candidate_posting_accumulation_{name}_cpu_posting_accumulation",
            comparison.get("cpu_i8_posting_accumulation_seconds"),
        )
        print_quiet_mean(
            f"gpu_i8_candidate_posting_accumulation_{name}_kernel",
            comparison.get("gpu_posting_accumulation_kernel_mean_seconds"),
        )
        print_quiet_mean(
...
def exit_code(report: dict[str, Any], *, gpu_available: bool, allow_missing: bool) -> int:
    ...
    if not gpu_available and not allow_missing:
        return 2
    if status == STATUS_BLOCKED_GPU_POSTING_ACCUMULATION_FAILED:
        return 3
    return 4
