...
from kayak_bridge.r2med_biology_late_interaction_rescoring import (
    benchmark_r2med_late_interaction_rescoring,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark pure late-interaction shortlist rescoring operators on "
            "the full R2MED/Biology snapshot."
        )
    )
    ...
    parser.add_argument(
        ...
        default=(
            ...
            / ".cache"
            / "kayak"
            / "r2med_biology_late_interaction_rescoring_summary.json"
        ),
    )
...
def main() -> None:
    args = parse_args()
    summary = benchmark_r2med_late_interaction_rescoring(
    ...
    print(
        "best operator="
        f"{best.operator_name}, "
        f"nDCG@10={best.mean_ndcg_at_k:.6f}, "
        f"MRR@10={best.mean_reciprocal_rank:.6f}, "
        f"rescore_s/query={best.rescoring_seconds_per_query:.6f}"
    )
