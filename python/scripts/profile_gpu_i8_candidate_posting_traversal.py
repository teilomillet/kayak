...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Profile GPU traversal of i8 selected-centroid posting lists "
            "without score accumulation."
        )
    )
    parser.add_argument(
        "--case",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(".cache/kayak/gpu_i8_candidate_posting_traversal/summary.json"),
    )
...
def print_quiet_sections(report: dict[str, Any]) -> None:
    ...
    for row in rows:
        ...
        if not isinstance(name, str) or not isinstance(comparison, dict):
            continue
        print_quiet_mean(
            f"gpu_i8_candidate_posting_traversal_{name}_cpu_candidate_generation",
            comparison.get("cpu_i8_candidate_generation_mean_seconds"),
        )
        print_quiet_mean(
            f"gpu_i8_candidate_posting_traversal_{name}_cpu_posting_accumulation",
            comparison.get("cpu_i8_posting_accumulation_seconds"),
        )
        print_quiet_mean(
            f"gpu_i8_candidate_posting_traversal_{name}_kernel",
            comparison.get("gpu_posting_kernel_mean_seconds"),
        )
        print_quiet_mean(
            f"gpu_i8_candidate_posting_traversal_{name}_selected_path",
            comparison.get("gpu_posting_selected_h2d_kernel_d2h_mean_seconds"),
        )
        print_quiet_mean(
            f"gpu_i8_candidate_posting_traversal_{name}_all_measured",
            comparison.get("gpu_posting_all_measured_mean_seconds"),
        )
...
def exit_code(report: dict[str, Any], *, gpu_available: bool, allow_missing: bool) -> int:
    ...
    if not gpu_available and not allow_missing:
        return 2
    if status == STATUS_BLOCKED_GPU_POSTING_TRAVERSAL_FAILED:
        return 3
    return 4
