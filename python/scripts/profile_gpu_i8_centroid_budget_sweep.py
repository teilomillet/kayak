...
from kayak_bridge.gpu_i8_centroid_budget_runner import build_report  # noqa: E402
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Sweep centroids-per-query-vector for CPU i8 candidate generation "
            "feeding the GPU rerank/top-k pipeline."
        )
    )
    parser.add_argument(
        "--case-set",
    ...
    parser.add_argument(
        ...
        help="Comma-separated centroids-per-query-vector budgets.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(".cache/kayak/gpu_i8_centroid_budget_sweep/summary.json"),
    )
...
def emit_quiet_means(report: dict[str, Any]) -> None:
    for row in report["cases"]:
        for budget_row in row["budget_rows"]:
            prefix = (
                "gpu_i8_centroid_budget_sweep_"
                f"{row['name']}_cpqv{budget_row['centroids_per_query_vector']}"
            )
            print_quiet_mean(
                f"{prefix}_candidate_generation",
                budget_row["cpu_i8_candidate_generation"]["mean_seconds"],
            )
            print_quiet_mean(
                f"{prefix}_candidate_plus_score",
                budget_row["comparison"][
                    "cpu_candidate_generation_plus_score_seconds"
                ],
            )
            print_quiet_mean(
                f"{prefix}_final_recall",
                budget_row["recall_at_k_vs_kayak_exact"],
            )
            breakdown = budget_row["candidate_generation_breakdown"]["aggregate"]
            print_quiet_mean(
                f"{prefix}_posting_visits",
                breakdown["posting_visit_count_total"],
            )
