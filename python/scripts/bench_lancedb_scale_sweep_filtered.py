...
from kayak_bridge.lancedb_benchmark import _filter_zero_vectors
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "the same encoded task after filtering zero vectors first."
        )
    )
    parser.add_argument("--task", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    parser.add_argument("--db-root", type=Path, required=True)
    parser.add_argument("--table-prefix", type=str, default="scale_sweep_filtered")
    parser.add_argument(
        "--target-document-count",
        type=int,
        action="append",
        dest="target_document_counts",
        help="Repeat to add multiple document-count points.",
    )
...
def main() -> None:
    ...
    filtered_task, *_ = _filter_zero_vectors(task)
    target_document_counts = args.target_document_counts
    if not target_document_counts:
        target_document_counts = _default_target_document_counts(filtered_task)
