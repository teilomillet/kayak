...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Profile GPU preparation/copy cost for i8 candidate-generation "
            "centroid-posting payload tensors."
        )
    )
    parser.add_argument(
        "--case",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(".cache/kayak/gpu_i8_candidate_generation_payload/summary.json"),
    )
...
def print_quiet_sections(report: dict[str, Any]) -> None:
    ...
    for row in rows:
        ...
        if not isinstance(name, str) or not isinstance(comparison, dict):
            continue
        print_quiet_mean(
            f"gpu_i8_candidate_generation_payload_{name}_cpu_candidate_generation",
            comparison.get("cpu_i8_candidate_generation_mean_seconds"),
        )
        print_quiet_mean(
            f"gpu_i8_candidate_generation_payload_{name}_extension_call",
            comparison.get("gpu_payload_extension_call_seconds"),
        )
        print_quiet_mean(
            f"gpu_i8_candidate_generation_payload_{name}_h2d",
            comparison.get("gpu_payload_host_to_device_mean_seconds"),
        )
        print_quiet_mean(
            f"gpu_i8_candidate_generation_payload_{name}_h2d_plus_d2h",
            comparison.get("gpu_payload_h2d_plus_d2h_mean_seconds"),
        )
...
def exit_code(report: dict[str, Any], *, gpu_available: bool, allow_missing: bool) -> int:
    ...
    if not gpu_available and not allow_missing:
        return 2
    if status == STATUS_BLOCKED_GPU_CANDIDATE_PAYLOAD_FAILED:
        return 3
    return 4
