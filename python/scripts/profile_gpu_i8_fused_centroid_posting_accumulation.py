...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Profile fused GPU i8 centroid scoring, device centroid "
            "selection, posting accumulation, and host document top-k."
        )
    )
    parser.add_argument(
        "--case",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            ".cache/kayak/gpu_i8_fused_centroid_posting_accumulation/summary.json"
        ),
    )
...
def print_quiet_sections(report: dict[str, Any]) -> None:
    ...
    for row in rows:
        ...
        if not isinstance(name, str) or not isinstance(comparison, dict):
            continue
        print_quiet_mean(
            f"gpu_i8_fused_centroid_posting_accumulation_{name}_cpu_candidate_generation",
            comparison.get("cpu_i8_candidate_generation_mean_seconds"),
        )
        print_quiet_mean(
            f"gpu_i8_fused_centroid_posting_accumulation_{name}_cpu_slice",
            comparison.get("cpu_i8_centroid_selection_posting_topk_seconds"),
        )
        print_quiet_mean(
            f"gpu_i8_fused_centroid_posting_accumulation_{name}_kernel_chain",
            comparison.get("gpu_fused_kernel_chain_mean_seconds"),
        )
        print_quiet_mean(
            f"gpu_i8_fused_centroid_posting_accumulation_{name}_resident_payload",
            comparison.get("gpu_fused_resident_payload_mean_seconds"),
        )
        print_quiet_mean(
            f"gpu_i8_fused_centroid_posting_accumulation_{name}_cold_payload",
            comparison.get("gpu_fused_cold_payload_mean_seconds"),
        )
...
def exit_code(report: dict[str, Any], *, gpu_available: bool, allow_missing: bool) -> int:
    ...
    if not gpu_available and not allow_missing:
        return 2
    if status == STATUS_BLOCKED_GPU_FUSED_CENTROID_POSTING_ACCUMULATION_FAILED:
        return 3
    return 4
