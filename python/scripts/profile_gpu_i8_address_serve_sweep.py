...
def print_quiet_sections(report: dict[str, Any]) -> None:
    ...
    for row in rows:
        ...
        print_quiet_mean(
            f"gpu_i8_address_serve_sweep_{name}_prepared_handle_score",
            comparison.get(
                "gpu_address_prepared_handle_score_extension_call_seconds_per_window"
            ),
        )
        print_quiet_mean(
            f"gpu_i8_address_serve_sweep_{name}_prepared_handle_topk",
            comparison.get(
                "gpu_address_prepared_handle_topk_extension_call_seconds_per_window"
            ),
        )
        print_quiet_mean(
            f"gpu_i8_address_serve_sweep_{name}_prepared_handle_topk_no_reference",
            comparison.get(
