"""Hybrid GPU i8 rows for FastPlaid scope comparisons.

This module assembles benchmark-only fused-shortlist plus exact-rerank rows.
Probe mechanics live in gpu_i8_fastplaid_hybrid_probe.
"""
...
from kayak_bridge.gpu_i8_fastplaid_hybrid_probe import (
    run_hybrid_shortlist_exact_rerank_probe,
)
...
def build_hybrid_shortlist_exact_rerank_scope_row(
    *,
    ...
    shortlist_k = _shortlist_k(args, shape)
    ...
    probe = run_hybrid_shortlist_exact_rerank_probe(
        capability=capability,
        shape=shape,
        query_windows=query_windows,
        payload=payload,
        index=index,
        fused_reference_score_windows=fused_reference_scores,
    ...
    return {
        "system_name": "kayak_gpu_i8_hybrid_shortlist_exact_rerank_probe",
        ...
        "scope": "real_kayak_i8_fused_shortlist_exact_rerank_topk",
...
def build_missing_hybrid_shortlist_exact_rerank_scope_row(
    *,
    shape: SpeedTrackShape,
    candidate_k: int,
    capability: MojoGpuCapability,
...
def _shortlist_k(args: argparse.Namespace, shape: SpeedTrackShape) -> int:
    requested = getattr(args, "gpu_hybrid_shortlist_k", None)
    if requested is not None:
        return int(requested)
    return min(int(args.candidate_k), int(shape.document_count))
