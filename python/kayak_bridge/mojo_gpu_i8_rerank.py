...
class MojoGpuI8SelectedPostingAccumulationResult:
    ...
    device_topk_kernel_mean_seconds: float
    device_topk_device_to_host_mean_seconds: float
    selected_position_out_of_range_count: int
    doc_index_out_of_range_count: int
    score_mismatch_count: int
    score_delta_max_abs: float
    topk_position_mismatch_count: int
    device_topk_position_mismatch_count: int
    top_k: int
    ...
    def accumulation_agreement_ok(self) -> bool:
        return (
            ...
            and self.device_topk_position_mismatch_count == 0
        )
    ...
    def to_json_ready(self) -> dict[str, object]:
        return {
            ...
            "device_topk_kernel_mean_seconds": (
                self.device_topk_kernel_mean_seconds
            ),
            "device_topk_device_to_host_mean_seconds": (
                self.device_topk_device_to_host_mean_seconds
            ),
            "selected_position_out_of_range_count": (
                self.selected_position_out_of_range_count
            ),
            ...
            "device_topk_position_mismatch_count": (
                self.device_topk_position_mismatch_count
            ),
            "top_k": self.top_k,
            "topk_position_count": self.topk_position_count,
...
@dataclass(frozen=True, slots=True)
class MojoGpuI8SelectedPostingResidentDenseCandidateResult:
    host_marshalling_seconds: float
    ...
    @property
    def score_plus_candidate_selection_seconds(self) -> float:
        return self.score_call_seconds + self.candidate_selection_seconds

    @property
    def prepare_score_plus_candidate_selection_seconds(self) -> float:
        return (
            self.prepare_call_seconds
            + self.score_call_seconds
            + self.candidate_selection_seconds
        )
    ...
    def to_json_ready(self) -> dict[str, object]:
        return {
            "host_marshalling_seconds": self.host_marshalling_seconds,
            "prepare_call_seconds": self.prepare_call_seconds,
            "score_call_seconds": self.score_call_seconds,
            "release_call_seconds": self.release_call_seconds,
            "candidate_selection_seconds": self.candidate_selection_seconds,
            "score_plus_candidate_selection_seconds": (
                self.score_plus_candidate_selection_seconds
            ),
            "prepare_score_plus_candidate_selection_seconds": (
                self.prepare_score_plus_candidate_selection_seconds
            ),
            "validation_seconds": self.validation_seconds,
            "document_score_count": self.document_score_count,
            "candidate_k": self.candidate_k,
...
@dataclass(frozen=True, slots=True)
class MojoGpuI8FusedCentroidPostingProfileResult:
    host_marshalling_seconds: float
    extension_call_seconds: float
    query_host_ingest_mean_seconds: float
    query_host_to_device_mean_seconds: float
    centroid_score_kernel_mean_seconds: float
    centroid_selection_kernel_mean_seconds: float
    accumulation_kernel_mean_seconds: float
    reduction_kernel_mean_seconds: float
    device_to_host_mean_seconds: float
    host_topk_restore_mean_seconds: float
    host_topk_restore_plus_destructive_mean_seconds: float
    host_topk_destructive_estimated_mean_seconds: float
    host_topk_non_destructive_mean_seconds: float
    document_score_count: int
    top_k: int
    topk_position_match_count: int
    topk_score_delta_max_abs: float
    ...
    def to_json_ready(self) -> dict[str, object]:
        return {
            "host_marshalling_seconds": self.host_marshalling_seconds,
            "extension_call_seconds": self.extension_call_seconds,
            "query_host_ingest_mean_seconds": self.query_host_ingest_mean_seconds,
            "query_host_to_device_mean_seconds": (
                self.query_host_to_device_mean_seconds
            ),
            "centroid_score_kernel_mean_seconds": (
            ...
            "reduction_kernel_mean_seconds": self.reduction_kernel_mean_seconds,
            "device_to_host_mean_seconds": self.device_to_host_mean_seconds,
            "host_topk_restore_mean_seconds": self.host_topk_restore_mean_seconds,
            "host_topk_restore_plus_destructive_mean_seconds": (
                self.host_topk_restore_plus_destructive_mean_seconds
            ),
            "host_topk_destructive_estimated_mean_seconds": (
                self.host_topk_destructive_estimated_mean_seconds
            ),
            "host_topk_non_destructive_mean_seconds": (
                self.host_topk_non_destructive_mean_seconds
            ),
            "document_score_count": self.document_score_count,
            "top_k": self.top_k,
            "topk_position_count": self.topk_position_count,
...
class MojoGpuI8FusedCentroidPostingSessionHandle:
    ...
    def profile_topk_without_reference(
        ...
        *,
        ...
        extension_started_at = time.perf_counter()
        raw_result = (
            module.profile_i8_fused_centroid_posting_session_handle_topk_no_reference(
                request
            )
        )
        extension_call_seconds = time.perf_counter() - extension_started_at
        ...
        if len(raw_result) != 15:
            raise RuntimeError(
                "GPU i8 fused centroid-posting top-k profile returned an "
                "unexpected result shape"
            )

        result_document_score_count = int(raw_result[11])
        result_top_k = int(raw_result[12])
        positions = tuple(int(value) for value in raw_result[13])
        scores = tuple(float(value) for value in raw_result[14])
        expected_topk_count = int(self.shape.query_count) * result_top_k
        if result_document_score_count != expected_score_count:
            raise RuntimeError(
                "GPU i8 fused centroid-posting top-k profile returned the "
                "wrong document score count"
            )
        if len(positions) != expected_topk_count or len(scores) != expected_topk_count:
            raise RuntimeError(
                "GPU i8 fused centroid-posting top-k profile returned the "
        ...
        return MojoGpuI8FusedCentroidPostingProfileResult(
            host_marshalling_seconds=host_marshalling_seconds,
            extension_call_seconds=extension_call_seconds,
            query_host_ingest_mean_seconds=float(raw_result[0]),
            query_host_to_device_mean_seconds=float(raw_result[1]),
            centroid_score_kernel_mean_seconds=float(raw_result[2]),
            centroid_selection_kernel_mean_seconds=float(raw_result[3]),
            accumulation_kernel_mean_seconds=float(raw_result[4]),
            reduction_kernel_mean_seconds=float(raw_result[5]),
            device_to_host_mean_seconds=float(raw_result[6]),
            host_topk_restore_mean_seconds=float(raw_result[7]),
            host_topk_restore_plus_destructive_mean_seconds=float(raw_result[8]),
            host_topk_destructive_estimated_mean_seconds=float(raw_result[9]),
            host_topk_non_destructive_mean_seconds=float(raw_result[10]),
            document_score_count=result_document_score_count,
            top_k=result_top_k,
            topk_position_match_count=topk_position_match_count,
            topk_score_delta_max_abs=score_delta_max_abs,
...
def profile_i8_selected_posting_accumulation_addresses(
    *,
    ...
    return MojoGpuI8SelectedPostingAccumulationResult(
        ...
        device_topk_kernel_mean_seconds=float(raw_result[6]),
        device_topk_device_to_host_mean_seconds=float(raw_result[7]),
        selected_position_out_of_range_count=int(raw_result[8]),
        doc_index_out_of_range_count=int(raw_result[9]),
        score_mismatch_count=int(raw_result[10]),
        score_delta_max_abs=float(raw_result[11]),
        topk_position_mismatch_count=int(raw_result[12]),
        device_topk_position_mismatch_count=int(raw_result[13]),
        top_k=int(raw_result[14]),
        topk_position_count=int(raw_result[15]),
...
def score_i8_selected_posting_candidate_positions_addresses(
    *,
    ...
    return _score_i8_selected_posting_candidate_positions_addresses(
        target_accelerator=target_accelerator,
        shape=shape,
        payload=payload,
        selected=selected,
        candidate_k=candidate_k,
        binding_name="score_i8_selected_posting_candidate_positions_addresses",
    )
...
def score_i8_selected_posting_block_candidate_positions_addresses(
    *,
    ...
    return _score_i8_selected_posting_candidate_positions_addresses(
        ...
        selected=selected,
        candidate_k=candidate_k,
        binding_name="score_i8_selected_posting_block_candidate_positions_addresses",
    )


def _score_i8_selected_posting_candidate_positions_addresses(
    *,
    target_accelerator: str,
    shape: Any,
    payload: KayakPlaidI8PayloadSnapshot,
    ...
    extension_started_at = time.perf_counter()
    raw_result = getattr(module, binding_name)(request)
    extension_call_seconds = time.perf_counter() - extension_started_at
...
def score_i8_selected_posting_resident_dense_candidate_positions_addresses(
    *,
    target_accelerator: str,
    shape: Any,
    payload: KayakPlaidI8PayloadSnapshot,
    ...
    score_delta_max_abs = float(
        np.max(
            np.abs(
                np.asarray(dense_scores, dtype=np.float32)
                - reference.expected_document_scores
            )
        )
    )
    ...
    return MojoGpuI8SelectedPostingResidentDenseCandidateResult(
        host_marshalling_seconds=host_marshalling_seconds,
        ...
        score_delta_max_abs=score_delta_max_abs,
        candidate_score_delta_max_abs=candidate_score_delta_max_abs,
        selected_position_out_of_range_count=int(raw_result[1]),
        doc_index_out_of_range_count=int(raw_result[2]),
        selected_centroid_count=int(raw_result[3]),
        document_count=int(raw_result[4]),
        positions=positions,
        scores=scores,
    )
