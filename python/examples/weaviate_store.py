...
def main() -> None:
    ...
    documents = encoder.encode_documents(
        ...
        [
            ...
            "weaviate can persist named multivectors",
            "kayak loads rows from weaviate and runs exact search locally",
        ],
    )

    with tempfile.TemporaryDirectory(prefix="kayak-weaviate-example-") as tmpdir:
        with kayak.open_store(
            "weaviate",
            persistence_path=Path(tmpdir) / "weaviate",
            collection_name="Doc",
            vector_name="colbert",
