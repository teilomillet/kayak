...
def long_document_hard_recall_candidate_budgets(
    document_count: Int
) -> List[Int]:
...
def append_long_document_hard_recall_summaries(
    ...
    for profile in default_long_document_hard_recall_profiles():
        var fixture = make_long_document_hard_recall_fixture(profile)
        var collection_root = ensure_one_segment_collection_mirror(
            Path(".cache/kayak/long_document_hard_recall_" + profile.slice_name),
            CollectionId("long-document-hard-recall-" + profile.slice_name),
            tenant_id,
            namespace_id,
            snapshot_id,
            1,
        ...
        for candidate_k in long_document_hard_recall_candidate_budgets(
...
def main() raises:
    ...
    append_long_document_hard_recall_summaries(summaries)

    var output_root = Path(".cache/kayak")
    makedirs(output_root, exist_ok=True)
    (output_root / "long_document_hard_recall_stage_aware_search.json").write_text(
        stage_aware_search_summaries_json(summaries)
    )
    print(
        "wrote ",
        String(output_root / "long_document_hard_recall_stage_aware_search.json"),
    )
