...
comptime LINEAR_OUTPUT_TOLERANCE = 1e-5


struct LatentProxyLinearTiled4Measurement(Copyable):
    ...
    var mean_flat_rows_tiled4_seconds: Float64
    ...
    def __init__(
        ...
        mean_flat_rows_tiled4_seconds: Float64,
    ):
        ...
        self.mean_flat_rows_tiled4_seconds = mean_flat_rows_tiled4_seconds


def write_measurements_tsv(
    path: Path, measurements: List[LatentProxyLinearTiled4Measurement]
) raises:
    var lines = String()
    lines += (
        "profile_name\tblock_index\tblock_input_dim\tblock_output_dim\tmean_row_list_seconds\tmean_flat_rows_seconds\tmean_flat_rows_tiled4_seconds\n"
    )
    for measurement in measurements:
        ...
        lines += "\t"
        lines += String(measurement.mean_flat_rows_tiled4_seconds)
        lines += "\n"
    path.write_text(lines)


def require_output_lists_close(
    kernel_name: String,
    read expected: List[VectorScalar],
    read observed: List[VectorScalar],
) raises:
    if len(expected) != len(observed):
        raise Error(kernel_name + " output length mismatch")

    for index in range(len(expected)):
        var difference = Float64(expected[index]) - Float64(observed[index])
        if difference < 0.0:
            difference = 0.0 - difference
        if difference > LINEAR_OUTPUT_TOLERANCE:
            raise Error(
                kernel_name
...
def require_block_variants_match(
    read inputs: List[List[VectorScalar]],
    ...
    for input_vector in inputs:
        ...
        require_output_lists_close(
            "flat_rows",
            expected,
            linear_block_output_flat_rows(input_vector, block, flat_rows),
        )
        require_output_lists_close(
            "flat_rows_tiled4",
            expected,
...
def append_measurement(
    mut measurements: List[LatentProxyLinearTiled4Measurement],
    ...
    measurements.append(
        LatentProxyLinearTiled4Measurement(
...
def main() raises:
    ...
    if len(projection.blocks) != 2:
        raise Error("linear tiled4 experiment requires exactly two blocks")
    ...
    require_block_variants_match(block0_inputs, block0, block0_flat_rows)
    require_block_variants_match(block1_inputs, block1, block1_flat_rows)

    var measurements = List[LatentProxyLinearTiled4Measurement]()
    ...
    def block0_flat_rows_tiled4_once() capturing raises:
        for token_vector in block0_inputs:
            bench_compiler.keep(
                linear_block_output_flat_rows_tiled4(
                    token_vector,
    ...
    var block0_flat_rows_tiled4_report = benchmark.run[
        block0_flat_rows_tiled4_once
    ](
        num_warmup_iters=LATENT_PROXY_PROFILE_WARMUP_ITERS,
        max_iters=LATENT_PROXY_PROFILE_MAX_ITERS,
        min_runtime_secs=LATENT_PROXY_PROFILE_MIN_RUNTIME_SECS,
    ...
    append_measurement(
        ...
        block0_flat_rows_tiled4_report.mean(),
    )
    ...
    def block1_flat_rows_tiled4_once() capturing raises:
        for token_vector in block1_inputs:
            bench_compiler.keep(
                linear_block_output_flat_rows_tiled4(
                    token_vector,
    ...
    var block1_flat_rows_tiled4_report = benchmark.run[
        block1_flat_rows_tiled4_once
    ](
        num_warmup_iters=LATENT_PROXY_PROFILE_WARMUP_ITERS,
        max_iters=LATENT_PROXY_PROFILE_MAX_ITERS,
        min_runtime_secs=LATENT_PROXY_PROFILE_MIN_RUNTIME_SECS,
    ...
    append_measurement(
        ...
        block1_flat_rows_tiled4_report.mean(),
    )

    var output_root = Path(".cache/kayak")
    makedirs(output_root, exist_ok=True)
    var output_path = output_root / "profile_latent_proxy_linear_tiled4_experiment.tsv"
    ...
    for measurement in measurements:
        print(
            ...
            " flat_rows_tiled4=",
            measurement.mean_flat_rows_tiled4_seconds,
        )
        ...
        print("== block ", measurement.block_index, " flat_rows_tiled4 ==")
        print("Mean: ", measurement.mean_flat_rows_tiled4_seconds)
