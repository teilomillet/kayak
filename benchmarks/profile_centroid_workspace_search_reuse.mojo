...
struct WorkspaceSearchReuseMeasurement(Copyable):
...
def append_measurement_tsv_line(
    mut out: String, read measurement: WorkspaceSearchReuseMeasurement
):
...
def write_measurements_tsv(
    path: Path, measurements: List[WorkspaceSearchReuseMeasurement]
) raises:
    var lines = String()
    lines += "dataset_name\tslice_name\tcandidate_generator_kind\tmode\tcandidate_k\tdocument_count\tquery_vector_count\tnominal_document_vector_count\tvector_dim\tmean_seconds\n"
...
def verify_plan_results_match(
    ...
    for judged_query in task.queries:
        ...
        if not hits_match(fresh_hits, reused_hits):
            raise Error(
                "workspace search benchmark observed final-hit mismatch for "
...
def append_plan_measurements(
    mut measurements: List[WorkspaceSearchReuseMeasurement],
    dataset_name: String,
    read task: JudgedTask,
    read snapshot: ResolvedCollectionSnapshot,
    read plan: SearchPlan,
    ...
    verify_plan_results_match(backend, task, snapshot, plan)

    print("== ", plan.candidate_generator.kind, " plain end-to-end ==")
    measurements.append(
        WorkspaceSearchReuseMeasurement(
            dataset_name.copy(),
    ...
    print("== ", plan.candidate_generator.kind, " reused end-to-end ==")
    measurements.append(
        WorkspaceSearchReuseMeasurement(
            dataset_name.copy(),
            task.slice_name.copy(),
...
def main() raises:
    var measurements = List[WorkspaceSearchReuseMeasurement]()
    var cache = ensure_browsecomp_plus_gold_real_subset_cache()
    var snapshot = load_profile_snapshot(
        "browsecomp_plus_gold_workspace_search_profile",
        Path(".cache/kayak/browsecomp_plus_gold_workspace_search_profile"),
        cache.stored_index,
    )
    ...
    var output_path = output_root / "profile_centroid_workspace_search_reuse.tsv"
